...
<head>
	<meta charset="UTF-8">
	<title>订单结算页</title>
	<link rel="stylesheet" type="text/css" href="css/order.css" media="screen" title="no title" charset="utf-8">
...
<!-- ====================导航======================= -->
	<div class="body">
        ...
        <div class="order cl">
        	...
        	<div class="book fl">书名</div>
        	<div class="name fl">收件人</div>
        	<div class="phone fl">电话</div>
        	<div class="add fl">收货地址</div>
        	<div class="num fl">订单编号</div>
        		<%
					List<Uorder> ps = (List<Uorder>)request.getAttribute("order");
					String username = user.getName();
					Uorder p = null;
					for(int i=0;i<ps.size();i++){
					  p = ps.get(i);
					  String username1 = p.getPerson();
					  if(username1.equals(username)){
					%>
						<div class="divdate-1 cl">
							<div class="bname fl">
								<%=p.getCode() %>
							</div>
							<div class="bperson fl">
								<%=p.getPerson() %>
							</div>
							<div class="bphone fl">
								<%=p.getPhone() %>
				...
				%>
				<div class="date">总价：<%=p.getStatus() %></div>
				<div class="tijiao"><a href="MoneyServlet?status=<%=p.getStatus()%>">结 算</a></div>
		</div>
</body>
</html>
